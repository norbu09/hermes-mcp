defmodule Hermes.Server do
  ...
  defp handle_initialize(params, context, state) do
    ...
    state =
      if state.handler_module do
        case state.handler_module.handle_client_capabilities(context, client_capabilities, state.handler_state) do
  ...
  defp handle_get_resource(resource_id, params, context, state) do
    if state.handler_module do
      ...
      # Default implementation
      case Enum.find(state.resources, fn resource_module -> resource_module.uri() == resource_id end) do
        nil ->
